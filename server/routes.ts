...
Assignment to solve:`;
      ...
      prompt = `You are an expert academic assistant. Provide a comprehensive, well-structured response to this assignment. Use proper formatting with headings, subheadings, and clear organization. Break content into readable paragraphs and use appropriate academic writing style.
...
IMPORTANT: If this problem requires graphs, charts, or data visualization, provide the data in the following format at the end of your response:
...
Generate realistic data points based on the scientific/mathematical principles in the assignment.`;
    ...
    // Extract multiple graph data if present
    const graphData: GraphRequest[] = [];
    if (needsGraph && accumulatedResponse.includes('GRAPH_DATA_START')) {
      try {
        const graphRegex = /GRAPH_DATA_START([\s\S]*?)GRAPH_DATA_END/g;
        let match;
        while ((match = graphRegex.exec(accumulatedResponse)) !== null) {
          try {
            const graphJson = match[1].trim();
            const parsedGraph = JSON.parse(graphJson);
            graphData.push(parsedGraph);
...
Generate realistic data points based on the scientific/mathematical principles in the assignment.`;
    ...
    throw new Error('Failed to process with OpenAI streaming');
  }
}
...
Generate realistic data points based on the scientific/mathematical principles in the assignment.`;
    ...
    throw new Error('Failed to process with DeepSeek streaming');
  }
}
    ...
    try {
      ...
      if (userId) {
        ...
        if (user.username !== 'jmkuczynski' && user.username !== 'randyjohnson' && (user.tokenBalance || 0) < totalTokens) {
          res.write(`data: ${JSON.stringify({ type: 'error', message: '🔒 You\'ve used all your credits. [Buy More Credits]', needsUpgrade: true })}\n\n`);
          return res.end();
        }
        ...
        const assignment = await authService.saveAssignment({
        ...
        // Free users get limited processing
        res.write(`data: ${JSON.stringify({ type: 'error', message: 'Please login for streaming support' })}\n\n`);
      }
      ...
      res.write(`data: ${JSON.stringify({ type: 'error', message: error instanceof Error ? error.message : 'Processing failed' })}\n\n`);
      res.end();
    }
  });
